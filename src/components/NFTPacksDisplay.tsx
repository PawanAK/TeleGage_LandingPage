...
export default function NFTPacksGallery({
  ...
  useEffect(() => {
    const fetchAllNFTPacks = async () => {
      ...
      try {
        ...
        const superpacksResponse = await fetch(
          "https://telegage-server-8lhd.onrender.com/api/nft-superpacks"
        );
        ...
        const communityPacksResponse = await fetch(
          "https://telegage-server-8lhd.onrender.com/api/nft-packs",
          {
            method: "POST",
            headers: {
  ...
  if (!communityId) {
    return (
      <div className="text-center py-10">Loading community information...</div>
    );
  }
